...
def exibir_nucleos():
    ...
    for nucleo in nucleos:
        print(nucleo)
    return input("\nEscolha um núcleo (nome ou número): ").strip().lower()

def exibir_professores_otica():
    print("\nProfessores deste núcleo:")
    profs = ["1. Alexandre Dodonov", "2. Caio Ribeiro"]
    for prof in profs:
        print(prof)
    return input("\nEscolha um professor (nome ou número): ").strip().lower()

def info_caio_ribeiro():
    print("\n--- Informações sobre Caio Ribeiro ---")
    print("\nContato: caio.ribeiro@unb.br")
    print('\nLattes: http://lattes.cnpq.br/6459107436866263')
    print('\nScholar: https://scholar.google.com/citations?user=mR3sBh0AAAAJ&hl=pt-BR&oi=ao')
    print("\n1. Artigos Recentes:")
    print("- Ribeiro, Caio C. Holanda, Vitoria A. de Souza, and Vitorio A. De Lorenci., Electric current quantum superpositions in superconducting circuits. Physics Letters A (2025): 130599.")
    print("- de Freitas, Miguel Citeli, Thaïs Nivaille, and Caio C. Holanda Ribeiro., Probing cosmic expansion through small anisotropies in a Bose-Einstein condensate analogue. arXiv preprint arXiv:2504.21236 (2025).")
    ...
    "\ninteressados em estudar o backreaction quântico em sistemas análogos e nas modificações oriundas de interações de longo alcance, como, por exemplo, "
    "\nocorre quando os bósons possuem momento de dipolo elétrico ou magnético. Este projeto é executado no âmbito do grupo de pesquisa Óptica Quântica Teórica.")
    print("")
    print("- Flutuações do vácuo quântico em sistemas do tipo Casimir")
    print("Descrição: Em distinção à concepção clássica de vácuo, a modificação do estado de vácuo de algum campo quântico produz efeitos observáveis, como "
    "\npor exemplo o celebrado efeito Casimir. Neste projeto propomos estudar o efeito Browniano quântico em sistemas do tipo Casimir, no qual partículas que interagem "
    "\ncom o campo adquirem dispersão em suas velocidades devido a transições entre estados de vácuo.")
